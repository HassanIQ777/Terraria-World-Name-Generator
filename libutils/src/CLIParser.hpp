...
class CLIParser
{
  public:
	...
	bool hasFlag(std::string flag);		// true if a certain flag(argument) exists in argv

	int getIndex(std::string arg);		 // returns the index of an argument in argv (-1 if argument doesn't exist), use it to find arguments

	std::string getArg(int index); 		// returns the argument at index

	std::string getValue(std::string arg); // returns value after flag, for example you can find filename after --file FILENAME like: getValue("--file")
...
bool CLIParser::hasFlag(std::string flag)
...
int CLIParser::getIndex(std::string arg)
...
std::string CLIParser::getArg(int index)
{
...
std::string CLIParser::getValue(std::string arg)
{
